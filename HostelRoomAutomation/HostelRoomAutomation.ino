...
static Switch *my_switch1 = NULL;  //Dynamic memory allocation
  ...
  if (strcmp(device_name, "RFID LOCK") == 0) {
    ...
    if (strcmp(param_name, "REMOVE RFID") == 0) {
      ...
      if (remove_button == true) {
        ...
        String read = readStringFromEeprom(0);
        Serial.print("String Length Before Removal - ");
        ...
        if (read.indexOf(tid) == -1) {
          ...
          Serial.print("String Length After Removal - ");
          Serial.println(read.length());
          Serial.println(read);
...
void setup() {
  ...
  //-------------------------------------------- Initialize RFID door lock with params
  my_lock.addNameParam();
  Param disp("display", "custom.param.display", value("Welcome to Quantum101"), PROP_FLAG_READ);
